...
class ImageGraph:
    ...
    def print_adjacency_matrix(self):
        ...
        for node in self.nodes:
            for edged in node.edges:
                matrix[node.index][edged] = 1
        for row in matrix:
            for value in row:
                print(value, end='') #add comment
            print()
        print()
    ...
    def bfs(self, start_index, color):
        ...
        self.reset_visited()
        queue = Queue()
        starting_var = self.nodes[start_index]
        starting_color = starting_var.color
        starting_var.visit_and_set_color(color)
        ...
        while not queue.is_empty():
            stationed_index = queue.dequeue()
            stationed_node = self.nodes[stationed_index]

            for index_edge in stationed_node.edges:
                adjacent = self.nodes[index_edge]
                if adjacent.color == starting_color and not adjacent.visited:

                # add comment:
                #     add comment 
                    adjacent.visit_and_set_color(color)
                    queue.enqueue(index_edge)
                    self.print_image()
    ...
    def dfs(self, start_index, color):
        ...
        self.reset_visited()
        # print initial state
        starting_color = self.nodes[start_index].color
        print("Starting DFS; initial state:")
        self.print_image()
        self._assist(start_index, color, starting_color)

    def _assist(self, index_node, color, starting_color):
        node = self.nodes[index_node]
        # add comment
        if node.visited or node.color != starting_color:
            return

        node.visit_and_set_color(color)
        self.print_image()
        for adj_index in node.edges:
            self._assist(adj_index, color, starting_color)
