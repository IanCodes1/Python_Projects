...
class Tree:
    ...
    def height(self, node=None):
        def _helper(node=None):
            if node == None:
                return -1
            l_height = _helper(node.lChild)
            r_height = _helper(node.rChild)
    ...
    def range(self):
        if not self.root:
            return None
        biggest_node = self.root
        while biggest_node.rChild:
            biggest_node = biggest_node.rChild
        ...
        if biggest_node.key == min_node.key:
            return 0
        return biggest_node.key - min_node.key
    ''''' ADD CODE HERE'''
    
    def level(self, level):
        store_nodes = []
        def _helper(node, curr_lvl):
            if node == None or curr_lvl > level:
                return store_nodes
            if curr_lvl == level:
                return [node]

            l_node = _helper(node.lChild, curr_lvl +1)
            r_node = _helper(node.rChild, curr_lvl +1)
    ...
    def left_side_view(self):
        ...
        while current is not None:
            l_nodes.append(current) # Issue fixed here... changed current.key to current
    ...
    def sum_leaf_nodes(self):
        def _helper(node):
            ...
            if node.lChild == None and node.rChild == None:
                ...
                return _helper(node.lChild) + _helper(node.rChild)
        return _helper(self.root)
        ''' ##### ADD CODE HERE ##### '''
    ...
    def tree_info(self):
        ...
        print("Tree left side view:", self.left_side_view()) # line update 7/17
